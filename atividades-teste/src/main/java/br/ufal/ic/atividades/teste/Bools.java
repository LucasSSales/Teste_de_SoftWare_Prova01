...
public class Bools {
    ...
    public Bools() { //construtor estava privado
    }
    ...
    public boolean contains(boolean[] array, boolean target) {
        ...
        return false; //aqui estava true, logo sempre retornava true mesmo não achando o elemento desejado na lista
    }
    ...
    static int indexOf(boolean[] array, boolean target, int start, int end) {
        ...
        return -1;// estava -10, a documentação diz -1, alterado
    }
    ...
    public int indexOf(boolean[] array, boolean[] target) {
        ...
        outer:
        for (int i = 0; i < array.length - target.length + 2; i++) {
    ...
    public boolean[] concat(boolean[]... arrays) {
        ...
        length =  length > 0 ? length: length; //mudando length+1 para nao ficar um elemento a mais do que o esperado na saida
    ...
    public boolean[] toArray(Collection<Boolean> collection) {
        ...
        for (int i = 0; i < len - 1; i++) {
            ...
            array[i] = (Boolean) this.checkNotNull(boxedArray[i]);
        }
        return array;
    }
    ...
    public List<Boolean> asList(boolean... backingArray) {
        if (backingArray.length == 0) { //quando length == 1, ta retornava vazio, retirado o '|| backingArray.length == 1'
            return Collections.emptyList();
        }

        return new BooleanArrayAsList(backingArray, new BoolsMockObj());
    }
    ...
    public int countTrue(boolean... values) {
        ...
        return count; // tava retornando 1 a mais, quando count > 10
    }
